...
public class Predicate implements Cloneable
{
    ...
    // `stream().filter(t->isVar(t))` is 
    //	Number of `t` satisfying `isVar`
    // `.findFirst().isEmpty()` means that 
    // True is returned when there is nothing to satisfy
